...
def test_is_valid_name(person_info: PersonInfoDTO) -> None:
    ...
    assert utils.is_valid_name(person_info.name)
    assert utils.is_valid_name(person_info.surname)
...
def test_is_valid_name_wrong_path() -> None:
    ...
    with pytest.raises(FileNotFoundError):
        utils.extract_person_info("input_files/persons.json")  # File does not exist
...
def test_invalid_names(name: str) -> None:
    """Testing if the function works."""
    assert not utils.is_valid_name(name), f"Failed for name: {name}"
...
def test_validate_name_with_none() -> None:
    ...
    with pytest.raises(ValueError, match="Value must be of type string"):
        utils.is_valid_name(None)  # type: ignore
...
def test_validate_name_with_numeric(name: int | float) -> None:
    ...
    with pytest.raises(ValueError, match="Value must be of type string"):
        utils.is_valid_name(name)  # type: ignore
